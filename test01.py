...
class TestIview:
    ...
    def test_goto_radio_page(self, driver):
        with allure.step("goto radio page"):
            driver.find_element(By.XPATH, "//div[@class='v-list-item-title' and contains(.,'Radio button')]").click()

        with allure.step("assert radio page loading"):
            WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.XPATH, "//h1[contains(.,'Radio buttons')]")))
    ...
    def test_click_windows_radio(self, driver):
        ...
        with allure.step("selecting windows radio"):
            driver.find_element(By.XPATH, "//label[@class='v-label v-label--clickable' and contains(., 'Radio Three')]").click()
            sleep(1)
